...
function onMessageArrived(r_message){// quand un message arrive
	try {
		...
		dataJson = "";
		console.log("Json non conforme");
	}

		if(dataJson.hasOwnProperty('E_Lux')&&(connected_flag==1)){ // si dans le json la clé 'E_Lux' existe...

			temps = Number(dataJson.temps);
			Eclairement = Number(dataJson.E_Lux);
			gauge1.value = Eclairement;// mets à jour la jauge éclairement
			$('#ECL1').html(Eclairement);// rajoute la valeur de l'éclairement dans l'élément HTML d' ID ECL1 
			
			//affiche le point sur le graphe
			if (AffichePoints) {// si on veut afficher les points sur la graphe
				//console.log("affiche point ok !");
				if (depart){tempsDepart = temps;depart=false};
				//console.log("affiche point depart ok !");
				temps = temps - tempsDepart;
				affichePoint();// affiche le point dans la courbe plotly.js
				
				// pour l'enregistrement des points
				csvdata.push([currencyFormatFR(temps),currencyFormatFR(Eclairement)]);
				if (csvdata.length > NbPointsMax){
					csvdata.shift();// enlève le Premier point 
				}
			}

		}
		else {
			if(dataJson.hasOwnProperty('LEDRouge')){ // si dans le json le champ 'LEDRouge' existe...
				RedSlider.value = dataJson.LEDRouge;
			}
			if(dataJson.hasOwnProperty('LEDVerte')){
				GreenSlider.value = dataJson.LEDVerte;
			}
			if(dataJson.hasOwnProperty('LEDBleue')){
				BlueSlider.value = dataJson.LEDBleue;
			}
		}
...
function MQTTconnect() {
	...
	clientId = "iot_21"+RandomString(8);// création d'un numéro d'identification pour le client, ce numéro doit être unique
